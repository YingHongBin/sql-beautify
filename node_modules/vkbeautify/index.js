	...
	if(tag == 0){
		for (let i = 0; i < text_list.length; i++){
			if(/       \,CASE WHEN/ig.exec(text_list[i])){   
				...
				var case_when_list = text_list[i].replace(/CASE/ig, '\nCASE').replace(/WHEN/ig,'\nWHEN').replace(/ELSE/ig,"\nELSE").replace(/\s{1,}END/ig,"\nEND").split("\n");
				var c = 0;
				var if_comment = 0;
				var case_cnt = 0
				var case_flag = 0
				while (c < case_when_list.length){
					if(c==0){
						...
						if (case_flag==1){
							...
							if(/WHEN/.exec(case_when_list[c]) || /ELSE/.exec(case_when_list[c])){
								text_final_case += '\n' + Array((case_cnt) * 8 + 6).join(' ') + case_when_list[c];
							}else if (/CASE/.exec(case_when_list[c])){
								case_cnt += 1;
								text_final_case += '\n' + Array((case_cnt) * 8 + 1).join(' ') + case_when_list[c];
								case_flag = 1;
							}else if (/END/.exec(case_when_list[c])){
								text_final_case += '\n' + Array((case_cnt) * 8 + 1).join(' ') + case_when_list[c];
								case_cnt -= 1;
							}
						}
...
vkbeautify.prototype.sql = function(text,uppercase,comma_location,bracket_char,as_loc_cnt) {
	...
	console.log('step7')
	console.log(step7)
	step7_1 = convert_lowercase(step7); //转换为小写
	step7_2 = order_comment(step7_1);//将步骤放在转换为小写的位置，如果开启了转换为小写后，将会执行注释对齐，on 与 and 缩进三个优化功能
	step7_3 = order_where(step7_2);
	step8 = order_on(step7_3);
